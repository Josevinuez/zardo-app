...
} from "../utils/shopify-admin.js";
import type { AdminApiContext } from "../utils/shopify-admin.js";
...
import type { ProductCreateInput, ProductVariantsBulkInput, MediaInput } from "../services/queries.js";
...
import {
  findProducts,
} from "../services/prisma-queries.js";
...
function sendWishlistResponse(res: Response, status: number, payload: Record<string, unknown>) {
  res.status(status).set(wishlistCorsHeaders).json(payload);
}

async function resolvePrimaryLocationId(admin: AdminApiContext) {
  try {
    const response = await getShopLocation(admin);
    const locationId: string | null = response?.data?.location?.id ?? null;
    if (locationId) {
      return locationId;
    }
...
export function createPublicApiRouter(shopify: ShopifyApp) {
  const router = Router();
  ...
  router.get("/auth-health", async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      let offlineInfo: Record<string, unknown> = { present: false };
      if (offline) {
      ...
      let graphqlTest: Record<string, unknown>;
      if (offline) {
        try {
  ...
  router.post("/session-purge", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const body = req.body || {};
  ...
  router.post("/wishlist", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const data = req.body ?? {};
  ...
  router.post("/test-webhook-simple", (req: Request, res: Response) => {
  ...
  router.post("/wheel/spin", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const cartToken = req.headers['cart-token'] as string | undefined;
...
export function createProtectedApiRouter(shopify: ShopifyApp) {
  const router = Router();
  ...
  router.post("/psa/import", async (req, res, next) => {
    try {
      ...
      if (isDevelopment) {
        shop = (req.query.shop as string) || process.env.DEV_SHOP || "zardotest.myshopify.com";
      ...
      const parseNumbers = (input: unknown): number[] =>
        parseList(input)
      ...
      const queueJobs = validJobs.map((job) =>
        queueParsePSA
          .createJob({
            shop,
            certNo: job.certNo,
    ...
    try {
      ...
      res.json({
        ...
        failedJobs: failedJobs.map((job) => {
          const failedJob = job as typeof job & { failedReason?: string };
          return {
            data: job.data,
  ...
  router.post("/troll/import", async (req, res, next) => {
    try {
      ...
      const items = rawItems.filter(
        (item): item is NonNullable<typeof item> => Boolean(item?.name),
      );
  ...
  router.post("/products/create-manual", async (_req, res, next) => {
    try {
      ...
      if (isDevelopment) {
        shop = (_req.query.shop as string) || process.env.DEV_SHOP || "zardotest.myshopify.com";
        console.log("🔧 Dev mode: Using shop:", shop);
      ...
      const variantInput: ProductVariantsBulkInput = {
      ...
      const variantsResponse = await createBulkVariants(
        admin,
        product.id,
        [variantInput],
      ...
      if (variantErrors.length > 0) {
        ...
        return res.status(400).json({
          success: false,
          error: variantErrors.map((e: any) => `${e.field}: ${e.message}`).join(", "),
        });
      }
      ...
      if (createdVariants.length === 0) {
        ...
        const variantLocationId =
          createdVariant?.inventoryItem?.inventoryLevels?.nodes?.[0]?.location?.id ?? locationID;
      ...
      try {
        ...
        console.log(`📡 Found ${publications.length} publications:`, publications.map((p: any) => p.name));
  ...
  router.post("/inventory/calculate-and-save", async (_req, res, next) => {
    try {
      ...
      if (isDevelopment) {
        const shop = (_req.query.shop as string) || process.env.DEV_SHOP || "zardo-plus.myshopify.com";
        console.log("🔧 Dev mode: Using shop:", shop);
        // In dev mode, we need to get the offline admin context
        const offlineContext = await getOfflineAdminContext(shopify, shop);
        if (!offlineContext) {
  ...
  router.post("/test-webhook", async (_req, res, next) => {
    try {
      ...
      const inventoryWebhook = webhooks.find((webhook: any) => webhook.topic === "INVENTORY_LEVELS_UPDATE");
