...
queueParsePSA.process(
  ...
  console.log(`PSA Job ${job.id} | PSA data found, proceeding with product creation`);
      ...
      if (!tokenCheckResponse.ok || tokenCheckData.errors) {
        console.error(
          `PSA Job ${job.id} | Offline access token invalid for shop ${shop}`,
          tokenCheckData.errors ?? tokenCheckData,
        );
        return done(
          new Error(`Invalid session detected for ${shop}. Please visit the app again to refresh access.`),
        );
      }
      ...
      const psa_variant: ProductVariantsBulkInput[] = [
        {
          price: Number(price),
