...
import { createBulkVariants, createProductWMedia, getProductExists, adjustInventory, getPrimaryLocationId } from "./queries.js";
/**
...
export class LotService {
     ...
     */
    static async recordDebtPayment(lotId, paymentData) {
     ...
     */
    static async getDebtPaymentHistory(lotId) {
     ...
     */
    static async getDebtPaymentStats(lotId) {
     ...
     */
    static async addProductToLot(lotId, productData) {
     ...
     */
    static async updateLotProduct(productId, data) {
     ...
     */
    static async deleteLotProduct(productId) {
     ...
     */
    static async convertProductToShopify(lotProductId, admin, defaultPrice) {
        ...
        try {
            ...
            const productRes = await createProductWMedia(admin, productInput, []);
            ...
            if (lotProduct.variants.length > 0) {
                ...
                const variantsRes = await createBulkVariants(admin, shopifyProductId, variantsInput, 'REMOVE_STANDALONE_VARIANT');
                ...
                for (const createdVariant of created) {
                    const createdTitle = createdVariant?.title;
                    const lotVariant = lotProduct.variants.find(v => v.variantName === createdTitle);
                    if (!lotVariant)
                        continue;
                    const invItemId = createdVariant?.inventoryItem?.id;
                    let locId = createdVariant?.inventoryItem?.inventoryLevels?.nodes?.[0]?.location?.id;
                    if (!locId) {
                ...
                const defaultVariantId = node?.id;
                if (defaultVariantId) {
                    const variantUpdate = { id: defaultVariantId };
                    ...
                    let locId = node?.inventoryItem?.inventoryLevels?.nodes?.[0]?.location?.id;
                    if (!locId) {
                        const resolved = await getPrimaryLocationId(admin);
                        if (resolved)
                            locId = resolved;
            ...
            return { success: false, message: 'Error creating Shopify product: ' + (err && typeof err === 'object' && 'message' in err ? err.message : String(err)) };
        }
    }
     ...
     */
    static async addVariantToProduct(lotProductId, variantData) {
     ...
     */
    static async updateLotProductVariant(variantId, data) {
     ...
     */
    static async deleteLotProductVariant(variantId) {
     ...
     */
    static async convertLotProductVariant(variantId, shopifyVariantId) {
        return prisma.lotProductVariant.update({
            where: { id: variantId },
            data: {
                isConverted: true,
    ...
    static async getLotStatistics() {
        const [totalLots, pendingLots, convertedLots, deliveredLots, totalCost, totalDebt,] = await Promise.all([
            prisma.lot.count(),
     ...
     */
    static async getMonthlyAnalytics(year = new Date().getFullYear()) {
        ...
        const monthlyData = allLotsForYear.reduce((acc, lot) => {
     ...
     */
    static async getYearlyAnalyticsSummary(year = new Date().getFullYear()) {
        const startOfYear = new Date(year, 0, 1);
        const endOfYear = new Date(year + 1, 0, 1);
     ...
     */
    static async getAllDebtPayments() {
        return prisma.debtPayment.findMany({
            include: {
                lot: {
                    select: {
     ...
     */
    static async searchExistingProducts(searchTerm) {
     ...
     */
    static async addProductToLotWithShopifyLink(lotId, productData) {
