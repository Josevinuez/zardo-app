...
const delay = (ms) => new Promise((resolve) => setTimeout(() => resolve(void 0), ms));
const retryFetch = (url, timeout, fetchOptions = {}, retries = 3, retryDelay = 1000) => {
...
export async function getItemToad(url, shop, entered_quantity = 1, entered_price = 0.0, type = "standard") {
    const response = await retryFetch(url, 10000, {}, 3, 1000);
    const data = await response.text();
    const dom = new JSDOM(data);
    const productName = dom.window.document.querySelector(".product-name")?.textContent;
    const productPrice = dom.window.document.querySelector("#sale-price")?.textContent;
    const productImage = dom.window.document
        .querySelector("#main-prod-img")
        ?.children[0].getAttribute("src") || "";
    const productCollection = dom.window.document.querySelector(".font-small.font-md-default")
...
export async function getCollectionItemsToad(url, shop) {
    ...
    const lastPageMatch = Number.parseInt(lastPageHTMLMatch?.match(/(\d+)/g)?.[0] || "1");
    const cardInfo = [];
