...
import type { ItemToad } from "./scrapper.server";
...
import {
	...
	productVariantUpdatePrice,
} from "./queries.server";
import { removeBg } from "./removebg.server";
import {
	uploadImageBuffer,
} from "./supabase.server";
import { sendNotification } from "./notification.server";

export type QueueParseTrollToadJobInput = {
	shop: string;
	itemIn: ItemToad;
...
export let queueParseTrollToad: Queue<QueueParseTrollToadJobInput>;
try {
	...
	queueParseTrollToad.process(
		1,
		async (job: Job<QueueParseTrollToadJobInput>, done: DoneCallback<null>) => {
			const {
				shop,
				itemIn: ItemFields,
				existingProductId,
			} = job.data as QueueParseTrollToadJobInput;
			...
			const product = await createProductWMedia(
				admin,
				input_variables,
				productMedia,
			);
			if (product.data) {
				if (
					product.data.productCreate.userErrors &&
