...
// UPS API response interfaces
export interface UPSTrackingData {
...
export class UPSService {
  private static readonly API_BASE = process.env.UPS_API_BASE || 'https://onlinetools.ups.com/api';
  private static readonly CLIENT_ID = process.env.UPS_CLIENT_ID;
  private static readonly CLIENT_SECRET = process.env.UPS_CLIENT_SECRET;
  ...
  private static accessTokenCache: { token: string; expires: Date } | null = null;
   ...
   */
  static async getTrackingInfo(trackingNumber: string): Promise<UPSTrackingResponse> {
    try {
      ...
      const trackingData: UPSTrackingData = {
        trackingNumber,
        ...
        events: shipment.package?.[0]?.activity?.map((activity: any) => ({
   ...
   */
  static async processBatchTracking(trackingNumbers: string[]): Promise<UPSTrackingResponse[]> {
    const results: UPSTrackingResponse[] = [];
  ...
  static async updateLotTracking(): Promise<void> {
    ...
    try {
      ...
      const trackingNumbers = pendingLots
        .map((lot: any) => lot.upsTrackingNumber)
      ...
      let errorCount = 0;
         ...
         if (result.success && result.data) {
           const lot: any = pendingLots.find((l: any) => l.upsTrackingNumber === result.trackingNumber);
           if (lot) {
            try {
              // Update lot status
              ...
              for (const event of result.data.events) {
                ...
                const existingEvent = await (prisma as any).trackingEvent.findFirst({
                  where: {
                    lotId: lot.id,
   ...
   */
  static async refreshLotTracking(lotId: string): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  private static mapUPSStatus(statusCode: string): string {
  ...
  static async getTrackingStatistics() {
    try {
      const stats = await (prisma as any).lot.groupBy({
        by: ['trackingStatus'],
        _count: true,
        where: {
          upsTrackingNumber: { not: null },
